  ...
  begin
    ...
    begin
      ...
      for I := 0 to ACBrConsultaCNPJ1.CNAE2.Count - 1 do
        ...
        if Form1.PageControl4.TabIndex  = 3 then begin
          //Passar as informações para os Edits do Emitente ACBR
          with Form1 do begin
            edtEmitCNPJ.Text          :=  ACBrConsultaCNPJ1.CNPJ;
            edtEmitIE.Text            :=  '';
            edtEmitRazao.Text         :=  ACBrConsultaCNPJ1.RazaoSocial;
            edtEmitFantasia.Text      :=  ACBrConsultaCNPJ1.Fantasia;
            edtEmitFone.Text          :=  ACBrConsultaCNPJ1.Telefone;
            ...
            edtEmitCodCidade.Text     :=  ACBrConsultaCNPJ1.IBGE_Municipio;
            edtEmitCidade.Text        :=  ACBrConsultaCNPJ1.Cidade;
            edtEmitUF.Text            :=  ACBrConsultaCNPJ1.UF;
          end;
          Close;
        end else if Form1.PageControl2.TabIndex = 1 then begin
          //Passar as informações para os Edits do Destinatario
          with Form1 do begin             //Adicionar os edits no destinatario
            if rgConsultaCNPJ.ItemIndex = 0 then begin
              edtDestCNPJ.Text          :=  ACBrConsultaCNPJ1.CNPJ;
              edtDestIE.Text            :=  '';
              edtDestRazao.Text         :=  ACBrConsultaCNPJ1.RazaoSocial;
              edtDestFantasia.Text      :=  ACBrConsultaCNPJ1.Fantasia;
              edtDestFone.Text          :=  ACBrConsultaCNPJ1.Telefone;
              edtDestCEP.Text           :=  ACBrConsultaCNPJ1.CEP;
              edtDestLogradouro.Text    :=  ACBrConsultaCNPJ1.Endereco;
              edtDestNumero.Text        :=  ACBrConsultaCNPJ1.Numero;
              edtDestComp.Text          :=  ACBrConsultaCNPJ1.Complemento;
              edtDestBairro.Text        :=  ACBrConsultaCNPJ1.Bairro;
              edtDestCodCidade.Text     :=  ACBrConsultaCNPJ1.IBGE_Municipio;
              edtDestCidade.Text        :=  ACBrConsultaCNPJ1.Cidade;
              edtDestUF.Text            :=  ACBrConsultaCNPJ1.UF;
            end else begin
              TransportaCNPJCPF.Text    :=  ACBrConsultaCNPJ1.CNPJ;
              TransportaIE.Text         :=  '';
              TransportaxNome.Text      :=  ACBrConsultaCNPJ1.RazaoSocial;
              TransportaxEnder.Text     :=  ACBrConsultaCNPJ1.Endereco;
              TransportaxMun.Text       :=  ACBrConsultaCNPJ1.Cidade;
              TransportaUF.Text         :=  ACBrConsultaCNPJ1.UF;
            end;
          end;
          Close;
        end;
